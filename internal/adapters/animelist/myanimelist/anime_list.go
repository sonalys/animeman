...
func convertEntry(in []AnimeListEntry) []animelist.Entry {
	...
	for i := range in {
		entry := animelist.NewEntry(
			...
			must.Must(time.Parse(timeFormat, in[i].AnimeStartDateString)),
			must.Must(time.Parse(timeFormat, in[i].AnimeEndDateString)),
			in[i].NumEpisodes,
			nil,
		)
...
func (api *API) GetCurrentlyWatching(ctx context.Context) ([]animelist.Entry, error) {
	...
	req := must.Must(http.NewRequestWithContext(ctx, http.MethodGet, path, nil))
	v := url.Values{
		"offset": []string{"0"},
		"status": []string{"1"},
	}
